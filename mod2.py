...
for model_name, model in models.items():
    ...
    # Validation croisee
    cv_scores = cross_val_score(grid_search.best_estimator_, X_train, y_train, cv=StratifiedKFold(5))
    ...
    results.append((model_name, accuracy, cv_scores.mean()))
    metrics_text += f"\n{model_name} :\n"
    metrics_text += f"Accuracy : {accuracy:.2f}\nF1 Score : {f1:.2f}\nPrecision : {precision:.2f}\nRecall : {recall:.2f}\n"
    metrics_text += f"Cross-Validation Score : {cv_scores.mean():.2f} (\u00b1{cv_scores.std():.2f})\n\n"
    metrics_text += report + "\n"
    ...
    # Graphique comparaison des métriques par modèle

    # ===== Comparaison des modèles =====
# Création d'un graphique comparatif des modèles
plt.figure(figsize=(10, 6))
results_df.plot(kind='bar', x='Model', y=['Accuracy', 'Cross_Validation_Mean'], color=['blue', 'orange'], alpha=0.7)
plt.title("Comparaison des Modèles - Accuracy et Cross-Validation")
plt.ylabel("Score")
plt.xlabel("Modèle")
plt.xticks(rotation=45, ha='right')
plt.legend(['Accuracy', 'Cross-Validation Mean'])
plt.tight_layout()

# Sauvegarde du graphique
plt.savefig(os.path.join(graphs_folder, 'comparison_models_metrics.png'))
plt.close()

    

# ===== Sauvegarde des Résultats =====
results_df = pd.DataFrame(results, columns=['Model', 'Accuracy', 'Cross_Validation_Mean']).sort_values(by='Accuracy', ascending=False)
...
with open(os.path.join(output_folder, 'metrics_results.txt'), 'w') as f:
    ...
    f.write("\n\nMeilleur Modèle:\n")
    best_model_name, best_model_accuracy, best_model_cv = results_df.iloc[0]
    f.write(f"{best_model_name}\n")
    f.write(f"Accuracy : {best_model_accuracy:.2f}\nCross-Validation Score : {best_model_cv:.2f}\n")

print("Le script est terminé. Consultez le dossier 'mod2' pour les résultats.")